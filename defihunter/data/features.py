...
def compute_ohlcv_features(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    """
    new_cols = {}
    new_cols['range'] = df['high'] - df['low'] + 1e-8
    new_cols['body_ratio'] = abs(df['close'] - df['open']) / new_cols['range']
    
    max_body = df[['open', 'close']].max(axis=1)
    min_body = df[['open', 'close']].min(axis=1)
    
    new_cols['upper_wick_ratio'] = (df['high'] - max_body) / new_cols['range']
    new_cols['lower_wick_ratio'] = (min_body - df['low']) / new_cols['range']
    
    new_cols['close_to_high'] = (df['high'] - df['close']) / new_cols['range']
    new_cols['close_to_low'] = (df['close'] - df['low']) / new_cols['range']
    
    # Anomaly tracking for universe filtering (Timeframe-aware rolling window)
    lookback = TimeframeHelper.get_bars('12h', timeframe)
    wick_series = pd.DataFrame({
        'u': new_cols['upper_wick_ratio'], 
        'l': new_cols['lower_wick_ratio']
    }).max(axis=1)
...
def compute_atr_and_emas(df: pd.DataFrame, timeframe: str = '15m', atr_period=14, ema_periods=[20, 55, 100]) -> pd.DataFrame:
    ...
    # ATR
    new_cols['atr'] = tr.rolling(window=atr_period).mean()
...
def compute_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'timestamp' in df.columns:
        ...
        new_cols['hour_sin'] = np.sin(2 * np.pi * ts.dt.hour / 24)
        new_cols['hour_cos'] = np.cos(2 * np.pi * ts.dt.hour / 24)
        new_cols['day_sin']  = np.sin(2 * np.pi * ts.dt.dayofweek / 7)
        new_cols['day_cos']  = np.cos(2 * np.pi * ts.dt.dayofweek / 7)
        # Historical DeFi pump windows (UTC)
        new_cols['is_asia_session']    = ((ts.dt.hour >= 0) & (ts.dt.hour < 6)).astype(int)
        new_cols['is_us_open_session'] = ((ts.dt.hour >= 13) & (ts.dt.hour < 17)).astype(int)
...
def compute_participation_features(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    if 'volume' in df.columns:
        new_cols['volume_ma_20'] = df['volume'].rolling(20).mean()
        new_cols['volume_ma_50'] = df['volume'].rolling(50).mean()
        
    if 'taker_buy_volume' in df.columns and 'taker_sell_volume' in df.columns:
        new_cols['volume_delta'] = df['taker_buy_volume'] - df['taker_sell_volume']
        new_cols['cvd'] = new_cols['volume_delta'].cumsum()
        
        # Delta as percentage of total volume
        total_vol = df['taker_buy_volume'] + df['taker_sell_volume'] + 1e-8
        new_cols['delta_pct'] = new_cols['volume_delta'] / total_vol
        new_cols['buying_pressure'] = new_cols['delta_pct'].rolling(10).mean()
        
        roller_mean = new_cols['volume_delta'].rolling(50).mean().shift(1)
        roller_std = new_cols['volume_delta'].rolling(50).std().shift(1) + 1e-8
        new_cols['cvd_zscore'] = (new_cols['volume_delta'] - roller_mean) / roller_std
        new_cols['cvd_slope_4'] = new_cols['cvd'].diff(4) / 4
        
        # GT #12: Taker Buy Ratio — market buyers vs total (real urgency signal)
        new_cols['taker_buy_ratio'] = df['taker_buy_volume'] / total_vol
        tbr_mean = new_cols['taker_buy_ratio'].rolling(50).mean().shift(1)
        tbr_std  = new_cols['taker_buy_ratio'].rolling(50).std().shift(1) + 1e-8
        new_cols['taker_ratio_zscore'] = (new_cols['taker_buy_ratio'] - tbr_mean) / tbr_std
        ...
        new_cols['taker_surge'] = (
            (new_cols['taker_ratio_zscore'] > 2.0) &
            (new_cols['taker_buy_ratio'] > new_cols['taker_buy_ratio'].shift(1)) &
            (new_cols['taker_buy_ratio'].shift(1) > new_cols['taker_buy_ratio'].shift(2))
        )
        
        # Orderbook Vacuum: CVD accelerating while volatility shrinks
        if 'volatility_velocity' in df.columns:
            new_cols['orderbook_vacuum'] = (
                (new_cols['cvd_slope_4'] > 0) &
                (new_cols['cvd_slope_4'] > new_cols['cvd_slope_4'].shift(1)) &
    ...
    if 'open_interest' in df.columns:
        ...
        new_cols['oi_delta_1'] = np.where(df['open_interest'].shift(h1_bars) != 0, 
                                    (df['open_interest'] / df['open_interest'].shift(h1_bars)) - 1, 
                                    0.0)
        new_cols['oi_delta_4'] = np.where(df['open_interest'].shift(h4_bars) != 0, 
        ...
        new_cols['oi_zscore'] = (df['open_interest'] - oi_mean) / oi_std
        
        # Price is flat (< 1% move in 4h) but OI is elevated → accumulation
        price_flat = df['close'].pct_change(h4_bars).abs() < 0.01
        oi_rising  = new_cols['oi_zscore'] > 1.5
        new_cols['oi_divergence_bull'] = price_flat & oi_rising
        
        # GT-NEW-2: Short squeeze setup — OI dropping + negative funding
        oi_dropping = new_cols['oi_zscore'] < -1.0
        if 'funding_rate' in df.columns:
            neg_funding = df['funding_rate'] < -0.001
            new_cols['short_squeeze_setup'] = (oi_dropping & neg_funding).astype(bool)
        else:
            new_cols['short_squeeze_setup'] = oi_dropping.astype(bool)
    else:
        new_cols['oi_zscore'] = 0.0
        new_cols['oi_divergence_bull'] = False
        new_cols['short_squeeze_setup'] = False
        
    if 'funding_rate' in df.columns:
        # Simple rolling average of funding to some spikes
        new_cols['funding_rate_ma_10'] = df['funding_rate'].rolling(10).mean()
        # Funding Velocity: Is the crowd getting aggressively long/short?
        new_cols['funding_velocity'] = df['funding_rate'].diff(3)
        
        # GT-NEW-5 prep: extreme negative funding flag (feeds into rules.py)
        new_cols['funding_extreme_short'] = (df['funding_rate'] < -0.0005).astype(bool)
...
def compute_pre_pump_profile(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    new_cols['accumulation_zone'] = (price_change_h1 < 0.02) & (volume_change_h1 > 0.3)
    
    # Volume acceleration: volume increasing bar-over-bar
    new_cols['volume_acceleration'] = df['volume'].diff(1) / (df['volume'].shift(1) + 1e-8)
    new_cols['volume_accel_h1_sum'] = new_cols['volume_acceleration'].rolling(h1_bars).sum()
    ...
    if 'atr' in df.columns:
        ...
        new_cols['quiet_expansion'] = atr_shrinking & (new_cols['volume_accel_h1_sum'].shift(1) > 0.5)
    else:
        new_cols['quiet_expansion'] = False
    
    # Volume Spike now (for sweep confirmation - GT #4)
    if 'volume_zscore' in df.columns:
        new_cols['volume_spike_now'] = df['volume_zscore'] > 2.0
    else:
...
def compute_squeeze_features(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    new_cols['bb_upper'] = bb_mid + 2 * bb_std
    new_cols['bb_lower'] = bb_mid - 2 * bb_std
    new_cols['bb_width'] = ((new_cols['bb_upper'] - new_cols['bb_lower']) / (bb_mid + 1e-8)).shift(1)  # shift: no lookahead
    
    # Keltner Channel (5h period, 1.5x ATR)
    if 'atr' in df.columns:
        new_cols['kc_upper'] = bb_mid + 1.5 * df['atr']
        new_cols['kc_lower'] = bb_mid - 1.5 * df['atr']
        
        # Squeeze = BB inside KC (extreme volatility compression)
        new_cols['in_squeeze'] = (new_cols['bb_upper'] < new_cols['kc_upper']) & (new_cols['bb_lower'] > new_cols['kc_lower'])
        
        # Squeeze Release = was in squeeze last bar, now not → momentum incoming!
        new_cols['squeeze_release'] = (~new_cols['in_squeeze']) & (new_cols['in_squeeze'].shift(1).fillna(False))
        
        # Duration in squeeze (bars)
        in_sq_int = new_cols['in_squeeze'].astype(int)
        new_cols['squeeze_duration'] = in_sq_int.groupby(
        ...
        new_cols['bb_width_pct'] = new_cols['bb_width'].rolling(rank_bars, min_periods=min(20, rank_bars)).rank(pct=True)
    else:
        new_cols['in_squeeze'] = False
        new_cols['squeeze_release'] = False
...
def compute_breakout_features(df: pd.DataFrame, timeframe: str = '15m', lookback_h=5, atr_threshold=0.5) -> pd.DataFrame:
    ...
    new_cols['rolling_high'] = df['high'].rolling(window=lookback).max().shift(1)
    new_cols['rolling_low'] = df['low'].rolling(window=lookback).min().shift(1)
    
    # Breakout condition
    new_cols['is_breakout_bar'] = (df['close'] > new_cols['rolling_high'])
    
    # Distance in ATRs
    if 'atr' in df.columns:
        new_cols['breakout_distance_atr'] = (df['close'] - new_cols['rolling_high']) / df['atr']
        
        # Advanced Retest Logic
        # Was there a breakout in the last 2.5h?
        recent_bars = TimeframeHelper.get_bars('2.5h', timeframe)
        new_cols['recent_breakout'] = new_cols['is_breakout_bar'].rolling(window=recent_bars).max().fillna(0)
        ...
        new_cols['retest_touch'] = (
            (new_cols['recent_breakout'] == 1) & 
            (df['low'] <= new_cols['rolling_high'] + (df['atr'] * atr_threshold)) & 
            (df['low'] >= new_cols['rolling_high'] - (df['atr'] * atr_threshold))
        )
        # BUG #7 FIX: genuinely test if price is holding the level (not falling back through)
        new_cols['retest_hold'] = new_cols['retest_touch'] & (df['close'] > new_cols['rolling_high'] - df['atr'] * 0.2)
        
        # Reclaim logic (fell back below, now surging above)
        new_cols['fakeout_down'] = (new_cols['recent_breakout'] == 1) & (df['close'] < new_cols['rolling_high'])
        new_cols['reclaim_ok'] = new_cols['fakeout_down'].shift(1).fillna(0).astype(bool) & (df['close'] > new_cols['rolling_high'])
    else:
        new_cols['retest_touch'] = False
        new_cols['retest_hold'] = False
        new_cols['reclaim_ok'] = False
        
    # Sweep logic (undercut low and close above it)
    new_cols['sweep_low'] = (df['low'] < new_cols['rolling_low']) & (df['close'] > new_cols['rolling_low'])
    
    # GT #4 FIX: sweep_reclaim_confirmed now requires volume spike confirmation
    volume_spike = df.get('volume_spike_now', pd.Series(True, index=df.index))
    new_cols['sweep_reclaim_confirmed'] = (
        new_cols['sweep_low'] & 
        (df['close'] > df['open']) & 
    ...
    new_range = df['high'] - df['low'] + 1e-8 # Recomputing if not in df yet, but compute_ohlcv adds it
    r_pct = df.get('range', new_range) / df['close']
    new_cols['range_pct'] = r_pct
    new_cols['tightness'] = r_pct.rolling(20).std().shift(1)
    
    # BUG #1 FIX: tight_threshold uses shift(1) so the current bar doesn't influence the threshold
    tight_threshold = new_cols['tightness'].rolling(100).quantile(0.3).shift(1)
    new_cols['is_base_expansion'] = new_cols['is_breakout_bar'] & (new_cols['tightness'] < tight_threshold)
    ...
    if 'cvd_zscore' in df.columns and 'ema_100' in df.columns:
        new_cols['silent_accumulation'] = (
            (new_cols['tightness'] < tight_threshold) & 
            (df['cvd_zscore'] > 1.5) & 
            (df['close'] > df['ema_100'])
        )
    elif 'quiet_expansion' in df.columns:
        # Fallback to GT #1 quiet_expansion if no CVD data
        new_cols['silent_accumulation'] = df['quiet_expansion'] & (df['close'] > df.get('ema_100', df['close'] * 0.95))
    else:
...
def compute_persistence_features(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    if 'ema_55' in df.columns:
        ...
        new_cols['trend_persistence_score'] = above_ema.groupby((above_ema != above_ema.shift()).cumsum()).cumsum() * above_ema
    else:
        new_cols['trend_persistence_score'] = 0
 
    # 2. Close to High Persistence (Quality of buying)
    if 'close_to_high' in df.columns:
        new_cols['close_to_high_persistence'] = df['close_to_high'].rolling(10).mean()
    else:
        new_cols['close_to_high_persistence'] = 0.5
    ...
    if 'rsi_14' in df.columns and 'ema_55' in df.columns:
        ...
        new_cols['exhaustion_risk_score'] = (
            (df['rsi_14'] / 100.0 * 0.4) + 
            (np.clip(ema_dist * 10, 0, 1) * 0.4) + 
            (np.clip(green_streak / 10, 0, 1) * 0.2)
        ) * 100.0
    else:
        new_cols['exhaustion_risk_score'] = 0.0
 
    # 4. Volume Persistence
    if 'volume_ma_20' in df.columns:
        high_vol = (df['volume'] > df['volume_ma_20']).astype(int)
        new_cols['volume_persistence_score'] = high_vol.groupby((high_vol != high_vol.shift()).cumsum()).cumsum() * high_vol
    else:
...
def compute_exit_decay_features(df: pd.DataFrame, timeframe: str = '15m') -> pd.DataFrame:
    ...
    if 'volume_zscore' in df.columns and 'close_to_high_persistence' in df.columns:
        new_cols['volume_climax_flag'] = (df['volume_zscore'] > 3.0) & (df['close_to_high_persistence'] < 0.3)
    else:
        new_cols['volume_climax_flag'] = False
    ...
    if 'upper_wick_ratio' in df.columns:
        ...
        new_cols['wick_exhaustion_flag'] = (wick_streak >= 2)
    else:
...
def compute_market_structure_break(df: pd.DataFrame, timeframe: str = '15m', lookback_h=5) -> pd.DataFrame:
    ...
    new_cols['prev_high'] = df['high'].shift(1).rolling(window=window).max()
    new_cols['prev_low'] = df['low'].shift(1).rolling(window=window).min()
